...
def format_chat_few_shot(row, few_shot_examples):
    ...
    if QUERY_TYPE == "few-shot":
        ...
        return f"{task_desc} {action_desc} News Content: {row['News Content']}"
    elif QUERY_TYPE == "zero-shot-si":
        return f"{task_desc_si} {action_desc_si} News Content: {row['News Content']}"
...
def query_openai(client, model, messages, max_retries=3):
    ...
    """
    outputs = []
    for prompt in tqdm(messages, desc="Generating headlines"):
        for attempt in range(max_retries):
            try:
                response = client.chat.completions.create(
                    ...
                    messages=[
                        {"role": "system",
                         "content": "You are an expert in Sinhala journalism and headline generation."},
...
if __name__ == '__main__':
    ...
    OUTPUT_FOLDER = os.path.join("outputs", "headline_generation", model_id, QUERY_TYPE)
