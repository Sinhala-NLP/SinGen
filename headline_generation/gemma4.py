...
# channel even with thinking disabled, e.g. <|channel>thought\n...<channel|>.
# Strip it so reasoning never enters the prediction; adjust tags if newer.
...
# --------------------------------------------------------------------------- #
def generate(model, processor, list_of_messages: List[list], batch_size: int, max_new_tokens: int) -> List[str]:
    outputs = []
...
# Pipeline
# --------------------------------------------------------------------------- #
def predict(model, processor, model_id, batch_size, max_new_tokens, test_size):
    ...
    log_gpu_memory()
    responses = generate(model, processor, df['chat'].tolist(), batch_size, max_new_tokens)
...
if __name__ == '__main__':
    ...
    log_gpu_memory()
    predict(model, processor, model_id, args.batch_size, args.max_new_tokens, args.test_size)
    log_gpu_memory()