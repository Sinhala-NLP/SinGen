...
def format_chat_few_shot(row, few_shot_examples):
    ...
    if QUERY_TYPE == "few-shot":
        ...
        return f"{task_desc} {action_desc} S: {row['Complex']}"
    elif QUERY_TYPE == "zero-shot-si":
        return f"{task_desc_si} {action_desc_si} S: {row['Complex']}"
...
def query_openai(client, model, messages, max_retries=3):
    ...
    """
    outputs = []
    for prompt in tqdm(messages, desc="Generating predictions"):
        for attempt in range(max_retries):
            try:
                response = client.chat.completions.create(
                    model=model,
                    messages=[
                        {"role": "system", "content": "You are an expert in Sinhala language simplification."},
