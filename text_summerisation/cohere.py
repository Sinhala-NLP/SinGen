...
def get_few_shot_examples(train_df, num_examples=3):
    """
    Get random few-shot examples from the training dataset
    """
...
def format_chat(row):
    ...
    task_desc_si = "ඔබ සිංහල භාෂාවේ ප්‍රවීණයෙකු ලෙස උපකල්පනය කරන්න. පහත සිංහල පුවත් ලිපියේ සංක්ෂිප්ත සාරාංශයක් ලබා දෙන්න. සාරාංශය මුල් පාඨයට වඩා බෙහෙවින් කෙටි විය යුතු අතර ප්‍රධාන අදහස් සහ ප්‍රධාන කරුණු අන්තර්ගත විය යුතුය."
    action_desc_si = "'Summary:' යන ප්‍රත්‍යයයෙන් පසුව පමණක් සාරාංශය ලබා දෙන්න. වෙනත් කිසිදු උපසර්ගයක් හෝ විස්තරයක් එක් නොකරන්න."
...
def predict():
    ...
    # Use a subset of test data for efficiency (you can change this)
    df = test_df
    ...
    if QUERY_TYPE in ["few-shot", "few-shot-si"]:
        ...
        few_shot_examples = get_few_shot_examples(train_df, num_examples=3)
    ...
    df['preds'] = df['responses'].apply(extract_summary)
