...
# --------------------------------------------------------------------------- #
# Few-shot + prompting (IDENTICAL wording to the Qwen summarisation script for
# ROUGE comparability — do not reword without updating every model script)
# --------------------------------------------------------------------------- #
...
def predict(model, tokenizer, model_id, batch_size, max_new_tokens, do_sample, test_size):
    ...
    if test_size and test_size > 0:
        df = df.head(test_size).copy()   # NOTE: XL-Sum Sinhala test is exactly 500 (the full set)
    ...
    with open(os.path.join(OUTPUT_FOLDER, "rouge_summary.txt"), 'w', encoding='utf-8') as f:
        ...
        f.write(f"Samples: {len(df)}\nDecoding: {'sampling(t=0.6,p=0.9)' if do_sample else 'greedy'}\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_size', type=int, default=0,
                        help='0 = full test set (XL-Sum Sinhala test is 500); else cap')
    parser.add_argument('--do_sample', action='store_true',
                        help='Llama sampling (t=0.6,p=0.9) instead of greedy. Leave off for ROUGE reproducibility.')
