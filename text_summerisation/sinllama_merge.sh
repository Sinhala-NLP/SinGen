...
for run in "slerp-a03:${MODELS}/SinLlama-slerp-a03" \
  ...
  for qt in zero-shot zero-shot-si few-shot few-shot-si; do
    echo "==================================================================="
    echo " ${tag} | summarisation (XL-Sum si) | query_type=${qt}"
    echo "==================================================================="
    python -m evaluate_summarisation_merged \
      --model_id "${path}" \
      --run_tag "${tag}" \
      --query_type "${qt}" \
      --batch_size 4 \
...
echo "All merged-checkpoint zero/few-shot summarisation runs finished."