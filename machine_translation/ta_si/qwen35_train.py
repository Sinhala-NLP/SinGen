...
def load_train_pairs(train_tsv, dev_df, train_size, dev_holdout):
    ...
    NOTE ON MODEL SELECTION: training on all of `dev` leaves nothing held out,
    so the learning rate, epoch count and LoRA rank are fixed a priori rather
    than selected. That is defensible only if stated. --dev_holdout carves a
    slice off the training pairs and reports loss on it, which turns "we did not
    tune" into "we tuned on a held-out slice"; either is fine for the paper, but
    ...
    eval_holdout = None
    if dev_holdout > 0:
        n = int(dev_holdout) if dev_holdout >= 1 else int(round(dev_holdout * len(train_df)))
        n = max(1, min(n, len(train_df) - 1))
        shuffled = train_df.sample(frac=1.0, random_state=777).reset_index(drop=True)
        eval_holdout = shuffled.head(n).reset_index(drop=True)
        train_df = shuffled.tail(len(shuffled) - n).reset_index(drop=True)
        print(f"Held out {len(eval_holdout)} pairs for loss monitoring; "
              f"{len(train_df)} pairs remain for training")

    return train_df, eval_holdout
...
def build_train_dataset(tok, train_df, prompt_lang, max_len, enable_thinking, desc="Building train examples"):
    end_id = turn_end_id(tok)
    examples, n_trunc, seq_lens = [], 0, []
    for _, row in tqdm(train_df.iterrows(), total=len(train_df), desc=desc):
...
# prompting runs; corpus sacreBLEU and spBLEU are reported alongside.
def tokenize(text):
...
def _clean_pairs(refs: List[str], preds: List[str]):
...
def corpus_sacrebleu(refs: List[str], preds: List[str]) -> Optional[float]:
    ...
    try:
        ...
        return None
    r, p = _clean_pairs(refs, preds)
    if r is None:
        return None
    return sacrebleu.corpus_bleu(p, [r], tokenize="none").score


def corpus_spbleu(refs: List[str], preds: List[str]) -> Optional[float]:
    """spBLEU -- sacreBLEU with the FLORES-200 SentencePiece tokenizer.

    This is the metric NLLB and essentially every published FLORES Sinhala
    result uses, so it is the only one of the three a reviewer can place next to
    prior work. It also sidesteps the 13a-mangles-Sinhala problem without
    needing the whitespace-tokenization argument defended.

    Caveat for the HPC run: the first call downloads the FLORES-200 SPM model.
    If the compute nodes have no outbound network this returns None -- warm the
    cache on the login node once with
        python -c "import sacrebleu; sacrebleu.metrics.BLEU(tokenize='flores200')"
    """
    try:
        import sacrebleu
    except ImportError:
        print("sacrebleu not installed -- skipping spBLEU.")
        return None
    r, p = _clean_pairs(refs, preds)
    if r is None:
        return None
    for tok_name in ("flores200", "flores101", "spm"):
        try:
            return sacrebleu.corpus_bleu(p, [r], tokenize=tok_name).score
        except Exception as e:      # unknown tokenizer name, or SPM download failed
            last = e
    print(f"spBLEU unavailable ({type(last).__name__}: {last}) -- skipping. "
          f"Warm the SPM cache on a networked node if this is a download failure.")
    return None
...
def evaluate(model, tok, test_df, model_id, prompt_lang, output_folder,
    ...
    spbleu = corpus_spbleu(df['sinhala'].tolist(), df['preds'].tolist())
    if corpus is not None:
        print(f"\nCorpus sacreBLEU (tokenize=none): {corpus:.4f}")
    if spbleu is not None:
        print(f"spBLEU (tokenize=flores200):      {spbleu:.4f}   <-- comparable to published work")
    ...
    with open(os.path.join(output_folder, "bleu_summary.txt"), 'w', encoding='utf-8') as f:
        ...
        if spbleu is not None:
            f.write(f"spBLEU (tokenize=flores200): {spbleu:.4f}  "
                    f"<- use this one for comparison with published FLORES results\n")
    ...
    return bleu_results, corpus, spbleu
...
def load_qwen(model_id, dtype, device_map):
    ...
    # A base (non-instruct) checkpoint has no chat template, and
    # apply_chat_template would raise several minutes into the run. Fail here
...
def select_lora_targets(model, config, attention_only, force_all_linear=False):
    ...
    `force_all_linear` is what --all_linear sets: it is the documented escape
    hatch from rule 2, so it has to be checked here rather than folded into
    `attention_only` by the caller (which would leave the guard free to flip it
    straight back on).
    """
...
# --------------------------------------------------------------------------- #
def write_model_card(path, repo_id, model_id, prompt_lang, bleu, corpus, spbleu, args, n_train):
    sent = bleu['bleu_overall']['mean'] if bleu else None
    # No hard-coded `license:` field. The adapter's licence follows the base
    # model, which varies across the checkpoints this script trains, and the
    # training data (FLORES-200) is CC-BY-SA 4.0 -- neither is apache-2.0 by
    # default, and asserting one on a public org repo is a claim we'd have to
    # stand behind.
    card = f"""---
language:
- si
...
FLORES-200 ships no training split, so training uses the `dev` split and evaluation
uses `devtest`. The two are document-disjoint (FLORES partitions 842 articles, not
individual sentences), so no sibling sentences leak between them -- but ~1k pairs is
a small supervised signal, and hyperparameters were fixed a priori rather than
selected on a validation set.

## Evaluation

FLORES-200 English-Sinhala `devtest` (1012 sentences):

| Metric | Score |
|---|---|
| spBLEU (`tokenize=flores200`) | {f'{spbleu:.2f}' if spbleu is not None else 'n/a'} |
| Corpus sacreBLEU (whitespace) | {f'{corpus:.2f}' if corpus is not None else 'n/a'} |
| Sentence-level BLEU mean | {f'{sent:.2f}' if sent is not None else 'n/a'} |

spBLEU is the figure to compare against published FLORES results. The whitespace
variants avoid sacreBLEU's default 13a tokenizer, which splits Sinhala conjuncts and
vowel signs; the sentence-level mean is retained only for comparability with the
other SinGen rows and is not corpus BLEU.

## Licence

This adapter inherits the licence of the base model -- check the base model card
before redistributing. The training and evaluation data is FLORES-200, released
under CC-BY-SA 4.0.
"""
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dev_holdout', type=float, default=0.0,
                        help='Hold out this many training pairs (>=1) or this fraction (<1) for '
                             'loss monitoring. 0 (default) trains on everything, which means no '
                             'validation set -- state that in the paper if you leave it at 0.')
    # training
    ...
    parser.add_argument('--no_save_adapter', action='store_true',
                        help='Skip writing the LoRA adapter to disk. (Replaces the old '
                             '--save_adapter, which had default=True and so could never be off.)')
    # hub
    ...
    # Fail before loading a 27B model rather than after every run has finished
    # and silently pushed nothing.
    ...
    if dev_df is None or devtest_df is None:
        ...
        raise SystemExit(1)
    train_df, holdout_df = load_train_pairs(args.train_tsv, dev_df, args.train_size, args.dev_holdout)
    ...
    if holdout_df is not None:
        holdout_ds = build_train_dataset(tok, holdout_df, prompt_lang, args.max_seq_len,
                                         args.enable_thinking, desc="Building holdout examples")
    ...
    if holdout_ds is not None:
        ta_kwargs.update(per_device_eval_batch_size=args.train_batch_size,
                         eval_strategy="epoch")
    ...
    # ------------------------------------------------------------------ eval
    bleu, corpus, spbleu = evaluate(
        model, tok, devtest_df, args.model_id, prompt_lang, OUTPUT_FOLDER,
    ...
    if args.push_to_hub:
        write_model_card(adapter_dir, hub_repo, args.model_id, prompt_lang,
                         bleu, corpus, spbleu, args, len(train_ds))
