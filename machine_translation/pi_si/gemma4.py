...
# the prompt). Kept IDENTICAL to the Llama Pali->Si script so prompts — and hence
# BLEU — stay comparable across model families. Only the demonstration examples
# are trimmed; the actual test instance is always passed in full.
FEWSHOT_PREVIEW_CHARS = 500


# ---------------------------------------------------------------------------
# Checkpoint routing (identical to the En->Si / Ta->Si Gemma scripts)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Data loading + split (from the Llama Pali->Sinhala script)
# ---------------------------------------------------------------------------
...
def split_dataset(full_df, test_size=1000):
    ...
    Identical split to the Llama script so families stay comparable.
    """
...
# ---------------------------------------------------------------------------
# Prompting (IDENTICAL wording to the Llama Pali->Sinhala script for BLEU
...
def query(model, proc, is_processor, is_gemma4, messages_list,
    ...
    """
    Runs greedy decoding in batches by default. Returns list of decoded strings
    with any reasoning blocks stripped. Uses left padding so new tokens start at
    the same offset for every sequence in a batch.
    """
    tok = get_tokenizer(proc, is_processor)
    ...
    # Gemma's recommended sampling settings; unused when do_sample is False.
...
def predict(model, proc, is_processor, is_gemma4, model_id, dev_df, test_df,
            max_new_tokens=512, batch_size=8, do_sample=False):
    print(f"Dev set size: {len(dev_df)}")
    print(f"Test set size: {len(test_df)}")
...
if __name__ == '__main__':
    ...
    # Load + split dataset (test size fixed at 1000, matching the Llama script)
    ...
    predictions, bleu_results = predict(
        model, proc, is_processor, is_gemma4, MODEL_ID, dev_df, test_df,
        max_new_tokens=MAX_NEW_TOKENS, batch_size=BATCH_SIZE, do_sample=DO_SAMPLE,
    )