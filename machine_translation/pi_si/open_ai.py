...
def format_chat_few_shot(row, few_shot_examples):
    ...
    if QUERY_TYPE == "few-shot":
        ...
        return f"{task_desc} {action_desc} P: {row['pali_text']}"
    elif QUERY_TYPE == "zero-shot-si":
        return f"{task_desc_si} {action_desc_si} P: {row['pali_text']}"
        ...
        for attempt in range(max_retries):
            try:
                response = client.chat.completions.create(
                    model=model,
                    messages=[
                        {"role": "system", "content": "You are an expert in Pali to Sinhala translation."},
...
if __name__ == '__main__':
    ...
    print(f"Dataset: sinhala-nlp/pali-sinhala (from HuggingFace)")
