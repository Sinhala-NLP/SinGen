...
# --------------------------------------------------------------------------- #
# Gemma specifics  (identical handling to train_translation_gemma.py, the
# Ta=>Si script -- only the task/data layer below differs)
# --------------------------------------------------------------------------- #
# All Gemma checkpoints in SinGen are *instruct* models, so we keep the chat
# template used by the prompting scripts rather than the Alpaca wrapper the
# SinLlama path needs. Three things differ from the Llama path:
#
...
# --------------------------------------------------------------------------- #
# Prompt text  (identical instruction wording to the En=>Si prompting scripts
# and to train_translation_qwen.py, so BLEU stays comparable across models)
# --------------------------------------------------------------------------- #
# NOTE: the user turn is assembled inline (`... explanations. E: <src>`) because
# that is what the En=>Si prompting and Qwen fine-tuning scripts do. The Ta=>Si
# Gemma script puts the source on its own line after a blank line. The wording
# is identical; only that whitespace differs, and it differs *between tasks*,
# not between models within a task -- which is the axis Table 2 compares on.
...
def build_user_message(english: str, prompt_lang: str) -> str:
    if prompt_lang == "si":
...
# --------------------------------------------------------------------------- #
# Data loading  (identical to the prompting / Qwen En=>Si scripts)
# --------------------------------------------------------------------------- #
...
def check_test_overlap(train_df, test_df):
    """dev/devtest are disjoint, so this is a no-op for the default setup. It
    matters when --train_tsv is used: public En-Si parallel corpora (CCMatrix,
    NLLB mined data, OPUS bundles) routinely contain FLORES sentences, which
    would put the test set straight into training."""
    overlap = set(test_df['english'].astype(str)) & set(train_df['english'].astype(str))
    if overlap:
        print(f"WARNING: {len(overlap)}/{len(test_df)} devtest English sentences also appear in "
              f"the training corpus. Use --drop_train_overlap to remove them.")
    else:
        print("No English-side overlap between training corpus and devtest.")
    return overlap
...
def build_train_dataset(tok, template_owner, train_df, prompt_lang, max_len, end_id, multimodal):
    ...
    for _, row in tqdm(train_df.iterrows(), total=len(train_df), desc="Building train examples"):
        prompt_text = apply_template(template_owner,
                                     build_user_message(str(row['english']), prompt_lang),
                                     multimodal)
...
# near 1-2. NOT corpus BLEU and not comparable to published FLORES figures.
# Kept unchanged so fine-tuned runs are comparable to the prompting runs;
# corpus sacreBLEU is reported alongside.
def tokenize(text):
...
def evaluate(model, tok, template_owner, test_df, model_id, prompt_lang, output_folder,
    ...
    for english in df['english']:
        text = apply_template(template_owner, build_user_message(str(english), prompt_lang), multimodal)
    ...
    with open(os.path.join(output_folder, "bleu_summary.txt"), 'w', encoding='utf-8') as f:
        ...
        f.write(f"Train/devtest English-side overlap detected: {n_overlap}\n")
        if corpus is not None:
...
# --------------------------------------------------------------------------- #
def check_hub_token(push_to_hub: bool):
    """Fail in seconds rather than after three epochs. A read-scoped token is
    enough to download the gated Gemma weights and train, so without this check
    the run dies only at the final upload."""
    if not push_to_hub:
        return
    token = os.environ.get("HF_TOKEN")
    if not token:
        raise SystemExit("--push_to_hub was given but HF_TOKEN is empty. Export a "
                         "WRITE-scoped token before launching.")
    try:
        from huggingface_hub import HfApi
        info = HfApi().whoami(token=token)
        role = ((info.get("auth") or {}).get("accessToken") or {}).get("role")
        if role and role not in ("write", "admin"):
            raise SystemExit(f"HF_TOKEN has role '{role}'; pushing to the Hub needs 'write'.")
        print(f"HF token OK (user={info.get('name')}, role={role or 'unknown'})")
    except SystemExit:
        raise
    except Exception as e:                     # noqa: BLE001
        print(f"Could not verify the HF token scope ({e}); continuing anyway.")
...
def write_model_card(path, repo_id, model_id, prompt_lang, bleu, corpus, args, n_train, n_test):
    ...
    train_source = args.train_tsv if args.train_tsv else "FLORES-200 `dev` split"
    card = f"""---
license: gemma
language:
- si
...
SinGen Sinhala text generation benchmark and evaluated on FLORES-200
...
FLORES-200 English-Sinhala `devtest` ({n_test} sentences), whitespace-tokenized
(sacreBLEU's default 13a tokenizer splits Sinhala conjuncts and vowel signs):
...
def push_adapter(adapter_dir, repo_id, private=False):
    ...
    HfApi().upload_folder(folder_path=adapter_dir, repo_id=repo_id, token=token,
                          repo_type="model", commit_message="Add Gemma En-Si translation LoRA")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--drop_train_overlap', action='store_true',
                        help='Remove training pairs whose English side appears in devtest. '
                             'Only relevant with --train_tsv; dev/devtest are already disjoint.')
    # training
    ...
    check_hub_token(args.push_to_hub)
    ...
    # ------------------------------------------------------------------ data
    # Loaded before the model so a bad path or a failed FLORES download fails
    # in seconds, not after a 60GB checkpoint download.
    ...
    overlap = check_test_overlap(train_df, devtest_df)
    if overlap and args.drop_train_overlap:
        before = len(train_df)
        train_df = train_df[~train_df['english'].astype(str).isin(overlap)].reset_index(drop=True)
        print(f"Dropped {before - len(train_df)} overlapping train pairs")

    # ------------------------------------------------------------------ load
    # device_map="auto" spreads 27B/31B across the available GPUs (naive model
    ...
    sample_prompt = apply_template(template_owner,
                                   build_user_message(str(train_df['english'].iloc[0]), prompt_lang),
                                   multimodal)
    ...
    # ------------------------------------------------------------------ eval
    bleu, corpus = evaluate(model, tok, template_owner, devtest_df, args.model_name, prompt_lang,
    ...
    if args.push_to_hub:
        write_model_card(adapter_dir, hub_repo, args.model_name, prompt_lang,
                         bleu, corpus, args, len(train_ds), len(devtest_df))
