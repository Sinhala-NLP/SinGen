...
def format_chat_few_shot(row, few_shot_examples):
    ...
    task_desc_si = "ඔබ ඉංග්‍රීසි සිට සිංහල භාෂා පරිවර්තනයේ ප්‍රවීණයෙකු වන්න. පහත ඉංග්‍රීසි වාක්‍යය (E) අර්ථය සහ සන්දර්භය ආරක්ෂා කරමින් නිවැරදිව සිංහලයට පරිවර්තනය කරන්න."
    ...
    if QUERY_TYPE == "few-shot":
        ...
        return f"{task_desc} {action_desc} E: {row['english']}"
    elif QUERY_TYPE == "zero-shot-si":
        return f"{task_desc_si} {action_desc_si} E: {row['english']}"
        ...
        for attempt in range(max_retries):
            try:
                response = client.chat.completions.create(
                    model=model,
                    messages=[
                        {"role": "system", "content": "You are an expert in English to Sinhala translation."},
